    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       ...
       boolean flag = userService.register(new User(id,name,pwd));
        ...
        if(true == flag){
            ...
            out.print("<script>alert('注册成功!');window.location.href='"+getServletContext().getContextPath()+"/login/login.jsp'</script>");
        }else{
            System.out.println("注册失败 ============ " );
            //页面跳转（login.jsp）带出提示信息--转发
            out.print("<script>alert('注册失败!');window.location.href='"+getServletContext().getContextPath()+"/login/Register.jsp'</script>");
        }
    }
    @Override
